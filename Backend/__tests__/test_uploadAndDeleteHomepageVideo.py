...
def setup_test_data(db_conn):
    ...
    user_data = (
        'user123', 'John', 'Doe', 'http://example.com/profile.jpg', None,
...
def post_homepage_video(url, video_path, form_data, expected_status_code):
    headers = {
...
# Function to simulate deleting a homepage video
def delete_homepage_video(url, data, expected_status_code):
    headers = {
...
# Test function for uploading and then deleting a homepage video
@pytest.mark.usefixtures("setup_test_data")
def test_upload_and_delete_homepage_video():
    upload_url = "http://localhost:3000/users/homePageVideo"
    delete_url = "http://localhost:3000/users/homePageVideo"
    
    # Upload a test video
    video_path = os.path.join('BE-Daytz', '__tests__', 'testvideos', 'testVideo1.mp4')
    form_data = {}
    response_upload = post_homepage_video(upload_url, video_path, form_data, 200)
    print("Response Data (Upload):", response_upload)  # Debugging print statement
    assert response_upload['videoUrl'], "Failed to upload video"
    
    # Extract video ID from the response
    video_id = response_upload['videoUrl'].split('/')[-1]

    # Delete the uploaded video
    delete_data = {}
    response_delete = delete_homepage_video(delete_url, delete_data, 200)
    print("Response Data (Delete):", response_delete)  # Debugging print statement
    assert response_delete['message'] == 'User updated successfully', "Failed to delete homepage video"
    ...
    # Check if 'videoUrl' is in the response and is set to null
    assert 'videoUrl' in user_data, "Response does not contain 'videoUrl'"
    assert user_data['videoUrl'] is None, "Failed to set videoUrl to null"
